...
def get_azure_llm():
    try:
        ...
        return AzureChatOpenAI(
            ...
            temperature=0.2 # 降低 temperature 讓 AI 的回答更嚴謹、不胡言亂語
        )
    except Exception as e:
...
請把所有要修的課程，依據先修邏輯（例如基礎課排大一二、高階課排大三四）塞入表格中。
每個年級的表格欄位必須為：【學期】|【課程名稱】|【學分數】|【課程屬性分類】|【科目資料來源】
* 課程屬性分類請填：不分系必修 / 專長模組課 / 院選修-錨定特定系 / 院選修-超前部署 / 校訂通識
* 科目資料來源請填：高瞻系規 / 文件A(輔系表) / 文件B(課規表) / 通識中心

四、 💡 多元畢業門檻與實習時程建議 (請精簡條列)
用最精煉的文字說明實習、出國交換與英文畢業門檻的建議通過時間點。
